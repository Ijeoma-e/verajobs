...
app.post('/api/assistant', upload.single('audio'), async (req, res) => {
  ...
  try {
    ...
    if (req.file && req.file.buffer) {
      ...
      parts.push({ text: "Please transcribe the audio above and treat it as the user's primary message. If you cannot hear anything, respond as Vera and ask the user to speak more clearly." });
    } else if (message) {
    ...
    if (!jsonMatch) {
      throw new Error("Could not find valid JSON in Gemini response: " + responseText);
    }
